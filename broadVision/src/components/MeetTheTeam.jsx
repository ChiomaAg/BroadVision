...
export default function MeetTheTeam( ) {
    ...
    const team = [
  {
    ...
    qualifications: "OD (UNIBEN), MSc Public Health (UNILAG)",
  },
  {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 max-w-7xl mx-auto">
        {team.map((member, i) => (
          <div key={i} className="text-center">
            ...
            {/* ROLE */}
            <p className="text-[#1434A4] font-medium">{member.role}</p>

            {/* BIO */}
            <p className="mt-2 text-gray-600 text-sm leading-relaxed px-3">
              {member.bio}
            </p>
          </div>
        ))}
